...
Only what genuinely needs a ruling. Each carries the recommended answer; `design.md` is
written as if the recommendation stands, so a different ruling means a revision there.

1. **Hostname and port.** `mqtt.pidgeiot.com`, DNS-only A/AAAA to the same VPS as loft,
   TLS on 8883, no plaintext listener. No separate staging hostname: staging verification
   runs a local broker against `dovecote-staging` (certificate mode needs no dovecote config
   change) and PSK mode against `wrangler dev`. Recommend: yes to all.
...
3. **Internal PSK route and secret naming.** Add `GET /internal/device-psk/:pigeon_id` as a
   neutral alias of `/internal/coap-psk/:pigeon_id` now (same handler, generalised to any
...
5. **Last Will.** Accept only on the session's own publish topics and bridge it as an ordinary
   publish on ungraceful disconnect (a will to `pigeons/<id>/telemetry` with
   `{"status":"offline"}` is the useful form). The alternative is a new dovecote "device
   offline" route. Recommend: no new route; bridge the will.

6. **MQTT 5 and the shell relay timing.** MQTT 5 lands in phase 4 (after the device samples),
   not before; mapping `POST /pigeons/:id/shell` onto `pigeons/<id>/shell/{cmd,output}` is
   also a phase 4 candidate (until then a shell request to a subscribed MQTT pigeon answers
   504 instead of 409). Recommend: phase 4 for both.
