...
//! packets, answers pings itself, and enforces the in-flight QoS 1 cap as
//! a protocol matter (`limits::RECEIVE_MAXIMUM`, v5 DISCONNECT 0x93, a
//! v3.1.1 grace ceiling before close). Publishes are bridged one at a time
//! in arrival order, so per-session ordering holds and a slow upstream
//! bounds memory at the cap. A writer half serialises acks, retained
