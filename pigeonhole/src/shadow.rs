...
//! INBOUND silence, two missed pongs reconnects. Inbound only: outbound
//! frames prove nothing (a half-open path absorbs writes into the send
//! buffer for minutes), so sending telemetry never resets the timer. Reconnect backs off 1 s to 60 s with jitter; close code
